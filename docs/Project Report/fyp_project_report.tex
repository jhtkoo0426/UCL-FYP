...
Things to improve:
- mention training grasping policies in physical simulators then transferring them to the real word (103, 105, 257, 258, 273, 278)
- data-driven approaches outperforms classical methods
- we aim to take thes best of both analytic (24) and data-driven methods (17,18, 154, 188, 265) to develop robust and scalabe grasping methods
...
In a previous study conducted by Alkhatib et al. \cite{9091340}, logistic regression was utilized to evaluate the quality of grasps. Specifically, a regularized logistic regression model was utilized to determine the robustness of grasps and predict their stability. The model's inputs were derived from a set of features measured at each joint of a robotic hand, which were subsequently utilized for training the logistic regression model.


\section{Logit Model for Grasp Classification}
\label{sec:4.1}
The fundamental assumption of this approach is that grasps can be dichotomously categorized as either successful or unsuccessful outcomes.
...
\end{equation}
For each seed pose, we generate a fixed number $n$ of random poses from the seed by adding a small amount of Gaussian noise $K_n$ with zero mean and unit variance to each dimension of $S_i$. We then scale down the applied noise by a factor of 0.01 to match the relative scales of objects and coordinate values in our Pybullet simulation. We denote the generated grasp poses $X_v$ as follows:
\begin{equation}
    X_v=\Big\{S_i+K_i:K_i\sim\mathcal{N}(0,1),\;i=1,2,3,...,n\Big\}
\end{equation}
