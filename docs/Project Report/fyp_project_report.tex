...
\label{sec:5.2.3.1}
In differential geometry, the two principal curvatures at a given point of a surface are the maximum and minimum values of the curvature as expressed by the eigenvalues of the shape operator at that point. They measure how the surface bends by different amounts in different directions at that point \cite{principalcurvature}.\\

To compute the principal curvatures of an object in Pybullet, we first extract the mesh data of the object via \verb|getMeshData()|, a Pybullet built-in function that returns mesh information of triangle meshes \cite{pybullet}, namely:
\begin{enumerate}
    \item \textit{indices}, the number of vertices found in the mesh; and
    \item \textit{vertices}, an array of 3-tuples describing the triangle meshes.
\end{enumerate}
We then calculate the top $k$ principal curvatures according to the following algorithm:
\begin{enumerate}
    \item For each vertex in the mesh, find its 10 nearest neighbors.
    \item Compute the covariance matrix of the neighbors.
    \item Compute the eigenvalues and eigenvectors of the covariance matrix.
    \item Sort the eigenvalues in descending order.
    \item Compute the principal curvatures as the reciprocals of the two largest eigenvalues.
\end{enumerate}

% Next, we compute the principal curvatures at each vertex of the mesh by iterating over all vertices and computing the covariance matrix of its neighboring vertices. The covariance matrix is then used to compute the eigenvalues and eigenvectors, which are sorted in descending order. The reciprocals of the top two eigenvalues are taken as the principal curvatures at the current vertex. Finally, the function returns the top $k$ principal curvatures as a numpy array.\\

Next, we determine whether the principal components are able to cluster the three object categories into their own respective groups. To do this, we concatenate the principal components with the 3 dimensions of the objects (refer to \hyperref[tbl:5.1]{Table 5.1}). Then, we normalize these features by column with mean 0 and variance 1. Finally, we perform Principal Component Analysis on the features and select the top $n$ components to visualise them. \hyperref[fig:5.1]{Figure 5.1} shows the results of these visualisations in 2D and 3D.

\vspace*{-0.5cm}
\begin{figure}[H]%
    \centering
    \subfloat[\centering Top $k=2$ components]{{\includegraphics[width=0.45\textwidth]{docs/Project Report/Media/principal_curvature_2d.png} }}%
    \qquad
    \subfloat[\centering Top $k=3$ components]{{\includegraphics[width=0.45\textwidth]{docs/Project Report/Media/principal_curvature_3d.png} }}%
...
\label{sec:5.2.1}
Based on our analysis results of our baseline approach in \hyperref[sec:4.7]{Section 4.7}, we concluded that under limited dimensionality circumstances, the Logistic Regression model fits the dataset with the highest accuracy using a convolutional neural network as a dimensionality reduction technique.\\

\section{Training}

\subsection{Preprocessing}
dataset normalisatoin with sklearn.standardscaler
