    ...
    def __init__(self, params, gripPath):
        ...
        if self.arm == "ur10":
            ...
            if self.gripperName == "threeFingers":
                self.robot_model = p.loadURDF("Robots/UR/ur10/ur10_3fing.urdf", self.start_pos, self.start_orientation, useFixedBase=1)
            ...
            self.serialManipulator = robot_modeling.DQ_SerialManipulator(self.DH, 'standard')
            if self.gripperName == "threeFingers":
                self.robot_model = p.loadURDF("Robots/KUKA/kuka_3fing.urdf", self.start_pos, self.start_orientation, useFixedBase=1)
            elif self.gripperName == "RG6":
        ...
        basekuka = DQ(np.array([robotOrientation[3], robotOrientation[0], robotOrientation[1], robotOrientation[2], 0, 0, 0, 0 ]))
        basekuka = basekuka * (basekuka.norm().inv())
        basekuka = basekuka + DQ([0, 0, 0, 0, 0, 0.5*(robotPos[0]+robotJointInfo[14][0]), 0.5*(robotPos[1]+robotJointInfo[14][1]), 0.5*(robotPos[2]+robotJointInfo[14][2])])*basekuka
